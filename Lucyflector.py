...
def main(args=None, configure_logging=False):  # pylint: disable=too-many-branches
    ...
    try:
        ...
        # Wenn die Info-Option gesetzt ist, gib die Spiegel-Infos aus
        if options.info:
    ...
    # Speichern der Mirrorlist, falls der --save-Parameter gesetzt ist
    if options.save:
        save_path = options.save  # Der Pfad, den der Benutzer angegeben hat
        try:
            # Öffne die Datei zum Schreiben im UTF-8-Encoding
            with open(save_path, 'w', encoding='utf-8') as handle:
                handle.write(mirrorlist)
            print(f'Mirrorlist wurde gespeichert unter: {save_path}')
            os.system("clear")
            os.system(f"\n\ncat {save_path}")
            print('Powerd by Lucy')
        except IOError as err:
            sys.exit(f'error: {err.strerror}')  # Fehler beim Speichern
    else:
        print(mirrorlist)  # Wenn kein --save-Parameter angegeben wurde, gib die Mirrorlist aus
